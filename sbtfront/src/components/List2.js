...
import { CONTRACT_ABI, CONTRACT_ADDRESS } from '../config/selfSBTconfig.js';
import useWeb3 from '../hooks/useWeb3';
import TextField from "@mui/material/TextField";
//import '../css/SBTList.css'
import '../css/List2.css';

const SBTList = ({sbtList}) => {
  return (
    ...
    <Stack
      ...
      sx={{
        mt:3,background: "linear-gradient(#bdc3c8 50%, #fff 50%)",
      }}
    >
      <Stack sx={{ width: 200, alignItems: "center", mt: 15 }}>
        ...
        <Box className = "main_a">
              <Stack direction="row">
                <div key = "main_title" className="box">
                {sbtList.map((sbt, index) => (
                  <div key = "index" className="box2">
                  <img className = "img"
                  src={sbt.tokenURI} alt =""></img>
                  <div className='box3'>
                    <p>account: {sbt.account}</p>
                    <p>username: {sbt.userName}</p>
                    <p>userInformation: <div className='temp'>{sbt.UserInformation}</div></p>
                  </div>
                </div>
                ))}
                </div>
    ...
    //             {sbtList.map((sbt, index) => (
    //               <div key={index}>
    //                 <img class="main_archive_img" src={sbt.tokenURI} alt=""></img>
    //                 <p>Owner: {sbt.account}</p>
    //                 <p>username: {sbt.userName}</p>
    //                 <p>userInformation: {sbt.UserInformation}</p>
    //               </div>
    //             ))}
    //           </div>
    //         </li>
      ...
      if(web3){
        ...
        const totalSupply = await deployed.methods.totalSupply().call();
          ...
          for(let i = 0; i < totalSupply; i++){
            const tokenURI = await deployed.methods.tokenURI(i).call();
            console.log("1")
            const account = await deployed.methods.ownerOf(i).call();
            console.log("2")
            const userName = await deployed.methods.getUserName(i).call();
            console.log("3")
            const UserInformation = await deployed.methods.getUserInformation(i).call();
            console.log("4")
          ...
          //console.log(deployed.methods.tokenURI(1), "tokenURI");
        //console.log(sbtList);
          setSBTList(sbtList);
        } 
      };

    }    loadSBTList();},[web3, account]);

  return(
    <div>
      {/* <h1>{account} SBT List</h1> */}
      <Stack
        sx={{
          height: "100%",
          maxWidth: 700,
